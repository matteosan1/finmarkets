...
from finmarkets import generate_dates, Interval
...
class Test_ShortRate(unittest.TestCase):
    def test_vasicek(self):
        ...
        dates = generate_dates(date(2023, 10, 20), Interval("1y"), Interval("1d"))
        res = []
        dt = 1/365
        for i in range(n):
        ...
        self.assertAlmostEqual(v.ZCB(r0, 0, Interval("1y").tau()), 0.9613, places=3)
    ...
    def test_cir(self):
        ...
        dates = generate_dates(date(2023, 10, 20), Interval("1y"), Interval("1m"))
